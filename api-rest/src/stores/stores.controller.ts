...
export class StoresController {
  ...
  public convertToEntityCreate(
    createStoreDto?: CreateStoreDto,
    createStoreDtoV2?: CreateStoreDtoV2,
  ): Store {
    const store = new Store();
    const obj = createStoreDtoV2 || createStoreDto;
    store.name = obj.name;
  ...
  create(@Body() createStoreDto: CreateStoreDto) {
    return this.storesService.create(
      this.convertToEntityCreate(createStoreDto),
    );
  }
  @Post('V2/stores')
  create2(@Body() createStoreDtoV2: CreateStoreDtoV2) {
    return this.storesService.create2(
      this.convertToEntityCreate(null, createStoreDtoV2),
    );
  }
  ...
  update(@Param('id') id: string, @Body() updateStoreDto: UpdateStoreDto) {
    return this.storesService.update(
      id,
      this.convertToEntityUpdate(updateStoreDto),
    );
  }
